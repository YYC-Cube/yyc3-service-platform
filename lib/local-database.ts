...
class LocalDatabase {
  private dbName = "zuoyou-ems-local"
  private dbVersion = 2
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("customers")) {
          ...
          customerStore.createIndex("company", "company", { unique: false })
          customerStore.createIndex("status", "status", { unique: false })
          customerStore.createIndex("assignedTo", "assignedTo", { unique: false })
        ...
        if (!db.objectStoreNames.contains("okrs")) {
          ...
          okrStore.createIndex("title", "title", { unique: false })
          okrStore.createIndex("owner", "owner", { unique: false })
          okrStore.createIndex("department", "department", { unique: false })
          okrStore.createIndex("quarter", "quarter", { unique: false })
  ...
  async create<T>(storeName: string, data: T): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(`数据创建成功: ${storeName}`, dataWithTimestamp)
        resolve((dataWithTimestamp as any).id)
      }
      request.onerror = () => reject(request.error)
    })
  }
      ...
      getRequest.onsuccess = () => {
        ...
        if (existingData) {
          ...
          updateRequest.onsuccess = () => {
            console.log(`数据更新成功: ${storeName}`, updatedData)
            resolve()
          }
  ...
  // 搜索功能
  async search<T>(storeName: string, query: string, fields: string[]): Promise<T[]> {
    const allData = await this.list<T>(storeName)
  ...
  // 批量操作
  async batchCreate<T>(storeName: string, dataList: T[]): Promise<string[]> {
    if (!this.db) throw new Error("数据库未初始化")
    ...
    return new Promise((resolve, reject) => {
      ...
      dataList.forEach((data) => {
        ...
        request.onsuccess = () => {
          ids.push((dataWithTimestamp as any).id)
          completed++
          if (completed === total) {
            console.log(`批量创建成功: ${storeName}, 数量: ${total}`)
            resolve(ids)
          }
        }
  ...
  async exportData(storeName: string): Promise<any[]> {
    return await this.list(storeName)
  }

  // 数据导入
  async importData(storeName: string, data: any[]): Promise<void> {
    if (!this.db) throw new Error("数据库未初始化")
    ...
    // 导入新数据
    await this.batchCreate(storeName, data)
  }
  ...
  // 数据恢复
  async restore(backupData: DatabaseSchema): Promise<void> {
    ...
    for (const store of stores) {
      if (store === "settings") {
        ...
        await this.importData(store, (backupData as any)[store])
      }
    }
  ...
  async getSettings(): Promise<SystemSettings> {
    const settings = await this.read<{ key: string; value: SystemSettings }>("settings", "system")
    return settings?.value || this.getDefaultSettings()
  }
  ...
  async addLog(log: Omit<ActivityLog, "id" | "timestamp">): Promise<void> {
    ...
    await this.create("logs", logData)
  }
  ...
  async addNotification(notification: Omit<Notification, "id" | "timestamp">): Promise<void> {
    ...
    await this.create("notifications", notificationData)
  }

  async markNotificationAsRead(id: string): Promise<void> {
    await this.update("notifications", id, { isRead: true })
  }

  async getUnreadNotifications(userId: string): Promise<Notification[]> {
    const allNotifications = await this.list<Notification>("notifications")
  ...
  private getDefaultSettings(): SystemSettings {
    return {
      company: {
        name: "言语云企业管理系统",
        logo: "/images/yanyu-cloud-logo.png",
        address: "中国深圳市南山区",
        phone: "400-888-8888",
        email: "contact@yanyu-cloud.com",
      },
      system: {
        version: "1.0.0",
        theme: "light",
